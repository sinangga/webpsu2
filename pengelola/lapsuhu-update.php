<?php include "../library/fungsi-tgl-indo.php";
                $id_lapsuhu= $_GET['id'];
                $sql=mysql_query("SELECT * FROM lap_suhu WHERE id_lapsuhu='$id_lapsuhu'");
                $data = mysql_fetch_array($sql);
?>
    <section class="content-header">
      <h1>
        Edit Data Laporan Temperatur
      </h1>
    </section> 
    <!-- Main content -->
              ...
              <!-- /.form group -->
                ...
                <div class="form-group">
                    <label>Rata-Rata</label>
                        <input class="form-control" value="<?php echo $data['rata']; ?>" name="rata" required oninvalid="this.setCustomValidity('Isi temperatur rata-rata')" oninput="setCustomValidity('')">
                </div>
                <div class="form-group">
                    <label>Maksimum (&deg;C)</label>
                        <input class="form-control" value="<?php echo $data['max']; ?>" name="max" required oninvalid="this.setCustomValidity('Isi temperatur maksimum')" oninput="setCustomValidity('')">
                </div>
                <div class="form-group">
                    <label>Minimum (&deg;C)</label>
                        <input class="form-control" value="<?php echo $data['min']; ?>" name="min" required oninvalid="this.setCustomValidity('Isi temperatur minimum')" oninput="setCustomValidity('')">
                        <input type ="hidden" value = "<?php echo $data['id_lapsuhu']; ?>" name ="id_lapsuhu">
                </div>
